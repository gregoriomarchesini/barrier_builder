...
class TaskScheduler {
    ...
    std::vector<bMsg> get_result_as_message() const {
        return bMsgs;
    }
    ...
    void optimize_barrier_function() {
        ...
        double epsilon = 0.00001; // small value to avoid numerical issues
        for (int l =1; l < betas.size(); ++l) {
            ...
            //print active sets
            std::cout << "Active tasks at beta_l = " << beta_l << ": ";
        ...
        for (const auto& barrier : barriers){
            sum_vec.push_back(barrier.r_var);
        ...
        for (size_t i = 0; i < barriers.size(); ++i) {
            ...
            std::cout << "Barrier " << i << ":b vector: " << *barriers[i].task.hypercube.b << std::endl;

            barriers[i].save_json();
        }

        // print x_0 
        // std::cout << "Initial state: " << *x_0->level() << std::endl;
        // print zeta_0
        std::cout << "Zeta_0: " << *zeta_0->level() << std::endl;

        //fill up message
        for (auto& barrier : barriers) {
            bMsg msg;

            auto slopes_val = barrier.slopes_var->level();
            auto gamma0_val = barrier.gamma0_var->level();
            auto r_val      = barrier.r_var->level();
            auto slack_val  = barrier.slack_var->level(); 


            msg.slopes    = new_vector_from_array_ptr(slopes_val);
            msg.gamma0    = new_vector_from_array_ptr(gamma0_val);
            msg.r         = new_vector_from_array_ptr(r_val)[0]     ;
            msg.slack     = new_vector_from_array_ptr(slack_val)[0] ;
            msg.b_vector  = new_vector_from_array_ptr(barrier.task.hypercube.b);
            msg.time_grid = barrier.time_grid;
            msg.task_id   = barrier.task.task_id;
            msg.edge_i    = barrier.task.agent_i;
            msg.edge_j    = barrier.task.agent_j;
    ...
    std::vector<bMsg>                 bMsgs;              // List of messages to be sent to the agents
